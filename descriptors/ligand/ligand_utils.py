...
def get_indices_for_element(element, elements):
    return [idx for idx, e in enumerate(elements) if e == element]
...
def read_xtb_log2(moldir,mol_len,ligand_indices):
    os.chdir(moldir)
    ...
    wbo_data = {}
    wbo_values = {'ligand_max': float('-inf'), 'ligand_min': float('inf'), 'ligand_mean': 0}
    if wbo_start is None or wbo_end is None:
        ...
        total_wbo_values = [] 
        # parse the wbo of ligand_max and ligand_min
        for ligand_idx in ligand_indices:
            for key, value in wbo_data.items():
                # Check if the key contains the ligand
                if ligand_idx == int(key.split('_')[-1]) - 1: #0 indices
                    for value_i, wbo_value in value.items():
                        if value_i == 'total':
                             wbo_value_float = float(wbo_value)
                             # Update max and min wbo_values
                             wbo_values['ligand_max'] = max(wbo_values['ligand_max'], wbo_value_float)
                             wbo_values['ligand_min'] = min(wbo_values['ligand_min'], wbo_value_float)
                             total_wbo_values.append(wbo_value_float)
        if total_wbo_values:
            wbo_values['ligand_mean'] = sum(total_wbo_values) / len(total_wbo_values)
    ...
    for lc,line in enumerate(lines_f):
        ...
        if "(HOMO)" in line:
            E_HOMO=float(line.split()[-2])
        if "(LUMO)" in line:
            E_LUMO=float(line.split()[-2])
    ...
    alphas_values = {'ligand_max': float('-inf'), 'ligand_min': float('inf'), 'ligand_mean': 0}
    covCN_values = {'ligand_max': float('-inf'), 'ligand_min': float('inf'), 'ligand_mean': 0}
    muls_values = {'ligand_max': float('-inf'), 'ligand_min': float('inf'), 'ligand_mean': 0}
    c6_values = {'ligand_max': float('-inf'), 'ligand_min': float('inf'), 'ligand_mean': 0}

    ligand_alphas = [alphas[idx] for idx in ligand_indices]
    ligand_covCN = [covCN[idx] for idx in ligand_indices]
    ligand_muls = [muls[idx] for idx in ligand_indices]
    ligand_c6 = [c6[idx] for idx in ligand_indices]
    
    if ligand_alphas:
        alphas_values['ligand_max'] = max(ligand_alphas)
        alphas_values['ligand_min'] = min(ligand_alphas)
        alphas_values['ligand_mean'] = sum(ligand_alphas) / len(ligand_alphas)
        covCN_values['ligand_max'] = max(ligand_covCN)
        covCN_values['ligand_min'] = min(ligand_covCN)
        covCN_values['ligand_mean'] = sum(ligand_covCN) / len(ligand_covCN)
        muls_values['ligand_max'] = max(ligand_muls)
        muls_values['ligand_min'] = min(ligand_muls)
        muls_values['ligand_mean'] = sum(ligand_muls) / len(ligand_muls)
        c6_values['ligand_max'] = max(ligand_c6)
        c6_values['ligand_min'] = min(ligand_c6)
        c6_values['ligand_mean'] = sum(ligand_c6) / len(ligand_c6)

    return(True, muls_values, alphas_values, covCN_values, c6_values, wbo_values, dip, HOMO_LUMO_gap, E_HOMO, E_LUMO) #occ_energies, virt_energies)
